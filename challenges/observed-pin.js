...
function getPINs(observed) {
  ...
  if (observed.length === 1) return adjacentDigits[observed];
  const newPINs = [];
  const partialPINs = getPINs(observed.slice(0, observed.length - 1));
  partialPINs.forEach(partialPIN => {
    adjacentDigits[observed[observed.length - 1]].forEach(newDigit => {
      newPINs.push(partialPIN + newDigit);
    });
  });
  return newPINs;
}
